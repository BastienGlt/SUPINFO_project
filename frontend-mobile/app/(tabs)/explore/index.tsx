...
export default function ExploreScreen() {
  ...
  // Charger la collection de l'utilisateur au focus
  useFocusEffect(
    useCallback(() => {
  ...
  const handleOpenCollection = useCallback((game: RawgGame) => {
  ...
  const handleSetStatut = useCallback(async (statut: string) => {
    ...
    try {
      if (existing) {
        await apiFetch(`/bibliotheque/items/${existing.id}`, {
          method: 'PUT',
          token,
        ...
        setCollectionMap((prev) => {
          ...
          next.set(gameId, { ...existing, statut });
          return next;
        });
      } else {
        const data = await apiFetch<{ item_id: number }>('/bibliotheque/items', {
          ...
          token,
          body: JSON.stringify({
            api_reference_id: gameId,
            titre: selectedGame.name,
            statut,
          }),
        });
        setCollectionMap((prev) => {
          const next = new Map(prev);
          next.set(gameId, { id: data.item_id, oeuvre_id: selectedGame.id, statut, api_reference_id: gameId });
          return next;
        });
      }
    ...
    return (
      ...
      >
        <View>
          ...
          {/* Bouton collection — visible uniquement si connecté */}
          {user && token && (
            <TouchableOpacity
              style={[
                styles.collectionBtn,
                { backgroundColor: inCollection ? colors.tint : colors.surface + 'cc' },
              ]}
              onPress={(e) => { e.stopPropagation(); handleOpenCollection(item); }}
              activeOpacity={0.8}
              hitSlop={4}
            >
              {inCollection ? (
  ...
  }, [colors, router, user, token, collectionMap, handleOpenCollection]);
...
const styles = StyleSheet.create({
  ...
  // Bouton collection sur la carte
  collectionBtn: {
    position: 'absolute',
    bottom: 6,
    right: 6,
