...
import {
  Star, Plus, Check, ChevronDown, Edit3, BookOpen,
  ChevronUp, X,
...
export default function GameDetailScreen() {
        ...
        loads.push(
          apiFetch<UserRating>(`/critiques/${id}/ratings/me`, { token })
            .then((r) => {
              ...
              setCritiqueContenu(r.contenu ?? '');
            })
            .catch(() => {}),
          apiFetch<BiblioItem[]>('/bibliotheque/items', { token })
            .then((items) => {
              const found = items.find(
                (i) => String(i.oeuvre_id) === String(id) || String(i.api_reference_id) === String(id)
              );
              setLibrairie(found ?? null);
            })
            .catch(() => {}),
        );
      }
  ...
  if (!game) {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: colors.background }}>
        <Text style={{ color: colors.icon, fontSize: 15 }}>Jeu introuvable</Text>
      </View>
    );
  }
    ...
    <>
      ...
      >
        ...
        {game.background_image ? (
          ...
          <View style={[styles.hero, styles.heroFallback, { backgroundColor: colors.tint + '18' }]} />
        )}
        ...
        <View style={styles.body}>
          ...
          <View style={styles.titleRow}>
            ...
            {game.metacritic != null && (
              <View style={[styles.metacriticBadge, { backgroundColor: metacriticColor(game.metacritic) + '20' }]}>
                <Text style={[styles.metacriticText, { color: metacriticColor(game.metacritic) }]}>
                  {game.metacritic}
                </Text>
              </View>
            )}
            ...
            <View style={styles.starsRow}>
              {[1, 2, 3, 4, 5].map((i) => (
                <Star
                  ...
                  strokeWidth={0}
                  color={colors.tint}
                  fill={i <= rawgStars ? colors.tint : colors.border}
                />
              ))}
              <Text style={[styles.ratingValue, { color: colors.icon }]}>
          ...
          <View style={styles.tagsRow}>
            ...
            {game.genres.slice(0, 3).map((g) => (
              <View key={g.id} style={[styles.tag, { backgroundColor: colors.tint + '14', borderColor: colors.tint + '30' }]}>
          ...
          {!!game.description_raw?.trim() && (
            <View>
              ...
              >
                {descExpanded
                  ? <ChevronUp size={14} color={colors.tint} strokeWidth={2} />
                  : <ChevronDown size={14} color={colors.tint} strokeWidth={2} />}
              ...
              <View style={styles.statsRow}>
                ...
                <View style={styles.statItem}>
                  ...
                  <Text style={[styles.statLabel, { color: colors.icon }]}>
                    {stats.total === 1 ? 'Critique' : 'Critiques'}
                  </Text>
                </View>
              </View>
            </View>
          ...
          {user && token && (
            <View style={styles.actionsCol}>
              {/* Bibliothèque */}
              <TouchableOpacity
                style={[styles.actionBtn, { borderColor: librairie ? colors.tint + '60' : colors.border, backgroundColor: librairie ? colors.tint + '10' : colors.surface }]}
      ...
      <Modal
        ...
        transparent
        animationType="slide"
        onRequestClose={() => setStatutModal(false)}
      >
        <TouchableOpacity
