...
const TYPE_CONFIG = {
    ...
    follow_request: { icon: UserPlus, color: '#f59e0b', label: 'souhaite vous suivre' },
};
        ...
        <div className="page-container">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    {unreadCount > 0 && <span style={{ background: 'var(--danger)', color: 'white', borderRadius: '10px', padding: '2px 10px', fontSize: '0.8rem', fontWeight: 700 }}>{unreadCount} non lue{unreadCount > 1 ? 's' : ''}</span>}
                </div>
                {unreadCount > 0 && (
                    <button onClick={handleMarkAllAsRead} style={{ background: 'none', border: '1px solid var(--border)', borderRadius: '8px', padding: '8px 16px', color: 'var(--text-muted)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', fontFamily: 'Rajdhani, sans-serif', fontWeight: 700, fontSize: '0.9rem' }}>
            ...
            {notifications.length === 0 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    {notifications.map(notif => {
                        ...
                        const isClickable = notif.type === 'follow' || notif.type === 'follow_request';
                        return (
                            <div key={notif.id} onClick={() => isClickable && handleClick(notif)}
                                style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '14px 18px', borderRadius: '10px', cursor: isClickable ? 'pointer' : 'default', background: notif.lu ? 'var(--bg-card)' : 'var(--primary-glow)', border: notif.lu ? '1px solid var(--border)' : '1px solid var(--primary)', transition: 'background 0.15s' }}>
                                <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: `${config.color}20`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                    <Icon size={18} style={{ color: config.color }} />
                                </div>
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    <div style={{ color: 'var(--text)', fontSize: '0.9rem' }}>
                                        <span style={{ fontWeight: 700, color: 'var(--primary)' }}>{notif.from_pseudo || notif.from_prenom || 'Quelqu\'un'}</span>
                                        {' '}{config.label}
                                    </div>
                                ...
                                {!notif.lu && <div style={{ width: '10px', height: '10px', borderRadius: '50%', background: 'var(--primary)', flexShrink: 0 }} />}
                            </div>
                        );
                    })}
                </div>
